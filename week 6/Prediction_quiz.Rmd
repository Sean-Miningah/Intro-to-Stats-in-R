...
<li> (answer)When predicting to `ames_train`, the RMSE for `model.AIC` is higher than the RMSE for `model.full`.  However, when predicting to `ames_test`, the RMSE for `model.full` is higher.
...
model.BIC <- stepAIC(model.full, trace=FALSE, direction="backward",k = log(nrow(ames_train)))
...
pred.test.MPM.non_coverage <- 1-pred.test.MPM.coverage
pred.test.MPM.non_coverage
```
